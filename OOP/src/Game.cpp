...
Game::Game(int width,int height,LoadandSave* file):WindowSize(width,height)
{
    ...
    {
        gWindow = SDL_CreateWindow("Metal Slug",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,1440,900,SDL_WINDOW_SHOWN); ///created window
        gRenederer = SDL_CreateRenderer(gWindow,-1,SDL_RENDERER_PRESENTVSYNC); ///created renderer
        hero =  new  Hero(gWindow,gRenederer,&ListofObjects); ///created hero
        ListofObjects.add(hero);///added hero to the list
        splashScreen = new SplashScreen(&e,gWindow,gRenederer,"data\\SplashScreen\\splash1.png",1440,900);///created Splash Screen
        win = new WinScreen(&e,gWindow,gRenederer,"data\\SplashScreen\\splash.png",1440,900);/// created win screen
        lose = new WinScreen(&e,gWindow,gRenederer,"data\\SplashScreen\\splash1.png",1440,900);///created lose screen
        menu = new Menu(&e,gWindow,gRenederer,"data\\SplashScreen\\menu.png",1440,900);///created game screen
        gs = new GameScreen(&e,gWindow,gRenederer,"data\\Backgrounds\\main.png",1440,900,(Hero*)hero,&ListofObjects);///created game screen
        pauseScreen= new PauseScreen (&e,gWindow,gRenederer,"data\\SplashScreen\\menu.png",1440,900);///created pause screen
...
void Game::GameLogic()
{
    ...
    if (EnemyKillCount <= 12)
    {
        if(OnScreenEnemies<2)
        {
            if(rand()%3==0)
            {
                ListofObjects.add(new Obstacles(gWindow,gRenederer,1,"data\\Obstacles\\armyTruck.png",1600,500,138,71,250,150,this->gs));
            }
    ...
    {
            ListofObjects.add(new Enemy(gWindow,gRenederer,1840,&ListofObjects,400));
            ListofObjects.add(new Enemy(gWindow,gRenederer,2040,&ListofObjects,600));
            ListofObjects.add(new Enemy(gWindow,gRenederer,2300,&ListofObjects,800));
            ListofObjects.add(new Enemy(gWindow,gRenederer,2540,&ListofObjects,1000));
            ListofObjects.add(new Enemy(gWindow,gRenederer,2740,&ListofObjects,300));
            ListofObjects.add(new Enemy(gWindow,gRenederer,2840,&ListofObjects,600));
            FinalEnemiesGenerated = true;
    }
    else if(FinalEnemiesGenerated && EnemyKillCount>=18 && !BossGenerated)
    {
