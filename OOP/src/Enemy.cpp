...
bool Enemy::fire(int frame)
        {
            if(!(frame%delay))
            {
                        ...
                        if (xDiff)
                        {
                            ...
                            if(Ex-Hx>0)
                               bullets->add(new Bullet(gWindow,grenderer,(EnemyStates[cs]->get_Position()).get_X(),(EnemyStates[cs]->get_Position()).get_Y()+25,-10,-slope,"ENEMYBULLET"));
                            else
                                bullets->add(new Bullet(gWindow,grenderer,(EnemyStates[cs]->get_Position()).get_X()+dRect->w,(EnemyStates[cs]->get_Position()).get_Y()+25,10,slope,"ENEMYBULLET"));
                            return true;
                        }
                        else
                        {
